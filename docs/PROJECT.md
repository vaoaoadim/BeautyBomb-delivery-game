...
7. A branded callout asks the player to collect the delivery reward; a small vertical cream flies from the courier roof to the girl.
8. Only after the product transfer completes does the existing reward flow open.
9. Demo mode displays a clearly fictional promo code; production mode requests a prize from the client's server.
...
The delivery finale keeps the deterministic route at `45,000 ms` and adds a separate `1,500 ms` presentation-only finish road after progress reaches 100%. Traffic scheduling ends `3,200 ms` before delivery so the last obstacle leaves naturally; the finish road disables spawning, collision, and lane input while road motion, courier drive frames, and bounded brand-color confetti continue. The existing finite-arrival parallax contract then stops the scene beside generated `DST-001` and `CHR-001` masters. `UI-016` and `UI-017` present the Russian thank-you and `<ЗАБРАТЬ>` interaction, and `PRD-003` flies from the courier's roof anchor to the girl's receiving anchor before the existing reward-flow placeholder becomes visible.
