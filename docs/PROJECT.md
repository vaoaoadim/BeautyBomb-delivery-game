...
The approved intro uses `UI-013` v2 for the Russian comic callout and `UI-014` for the centered three-color `ЖМИ` prompt. UI-013 v2 preserves the approved callout and removes only the small yellow decorative mark in its lower-right interior. Its `ready → transition → playing` flow is integrated and verified at logical `360 × 640` and CSS widths of approximately `320` and `360 px`; no runtime font download is required.
