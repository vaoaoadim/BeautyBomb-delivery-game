...
The delivery finale keeps the deterministic route at `45,000 ms` and adds a separate `1,500 ms` presentation-only finish road after progress reaches 100%. Traffic scheduling ends `3,200 ms` before delivery so the last obstacle leaves naturally; the finish road disables spawning, collision, and lane input while road motion, courier drive frames, and bounded brand-color confetti continue. The existing finite-arrival parallax contract then stops the scene beside generated `DST-001` and `CHR-001` masters. `PRD-003` first flies from the courier's roof anchor to the girl's receiving anchor; only after it disappears do `UI-016` and `UI-017` present the Russian thank-you and `<ЗАБРАТЬ>` interaction. Claiming then opens the existing reward-flow placeholder.
