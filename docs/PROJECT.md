...
The approved courier static PNG now replaces the greybox vehicle in the Phaser scene. Its documented body collider excludes the roof-mounted Waterbomb tube. Three static obstacle candidates are integrated, the schedule guarantees an escape lane, and all vehicles now share the approved far/middle/near scales of `0.88`, `0.94`, and `1.00` for both rendering and collision. The next gate is owner approval of `visual-references/vehicle-lane-scale-check-v1.png` and the local playtest before courier or traffic animation begins. Prize roulette and client-site integration remain later phases.
