...
The existing pause flow now presents `UI-015` v1: a centered tail-free lavender comic callout that uses the same local Press Start 2P font as the intro for `Не тормози! Нужно успеть вовремя :)`. Existing `ПРОДОЛЖИТЬ` and `ЗАНОВО` controls keep their implementation and pressed behavior, appearing below the copy; pausing, resuming, restarting, keyboard handling, and gameplay authority are unchanged.
