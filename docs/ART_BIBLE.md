...
- `CHR-003` v1 is the approved full-body blonde recipient in a simple pink dress, authored as a compact `112 × 176` low-poly/pixel-art hybrid master and exported once into a `28 × 44` canvas. Large faceted color planes, minimal facial detail, simplified hair, hands, clothing, and shoes keep the `16 × 40` visible silhouette readable at runtime. For this asset-integration stage she retains `x=290, y=324`, origin `(0.5, 1)`, the existing baseline, and the existing left-facing delivery orientation. She remains separate from the city texture so her depth and product-receiving anchor stay controllable.
