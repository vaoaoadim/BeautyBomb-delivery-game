...
- `UI-010` v2 and `UI-012` v2 are compact top-right `32 × 32` yellow utility controls with black icons, a dark stepped frame, and restrained pink extrusion. Pause sits at `x=332, y=32`; sound sits directly below at `x=332, y=84`. Both use `44 × 44` hit areas; the visible gap and hit areas remain separate from the title and progress bar. Pause uses the existing pause flow, while sound retains the visual-only placeholder contract. Keyboard `P` and `Esc` retain their existing pause entry point.
