...
- Reuse the actual `360 × 640` gameplay composition in a frozen `ready` state: sky, coherent city, road, HUD, courier, lower console, and lane buttons retain their normal positions, while parallax, traffic, route time, and drive frames remain stopped.
...
- During ordinary waiting, `ЖМИ` uses a restrained `1.00 → 1.06 → 1.00` `Sine.InOut` pulse over `1,000 ms`; the courier uses only a subtle `1 px` idle bob and static drive frame `0`.
- A first canvas press, `Enter`, or `Space` triggers one guarded `1,000 ms` three-color transition. Route time remains zero until completion, then the callout is hidden and standard gameplay motion and utility controls resume.
- Under `prefers-reduced-motion`, remove CTA scaling and courier bob while retaining the static instruction and a calm color change before play.
- Keep the bottom console and lane controls visible but inert before play; hide pause, exit, and sound controls until the run starts. Do not add a fullscreen shade, separate START button, or the former start popup.
...
- `UI-010` and `UI-011` are matching `32 × 32` top-corner controls at `x=332/28, y=32`, each with a `12 px` horizontal safe inset. The right control pauses the run; the left open-door control remains a visual exit placeholder until host integration exists.
- `UI-012` is a compact `28 × 28` sound placeholder attached to the control console's right edge at `x=346, y=540`; it must remain inside the lower panel and outside the driving field.
- The title stays horizontally centered at `x=180` and sits `6 px` below the utility-button centerline at `y=38`.
