...
- `UI-010` v2 and `UI-012` v2 are compact top-right `32 × 32` yellow utility controls with black icons, a dark stepped frame, and restrained pink extrusion. Pause sits at `x=332, y=32`; sound sits directly below at `x=332, y=78`. Both use `44 × 44` hit areas; the visible gap and hit areas remain separate from the title and progress bar. Pause uses the existing pause flow, while sound retains the visual-only placeholder contract. Keyboard `P` and `Esc` retain their existing pause entry point.
...
- `CHR-001` v2 is a full-body blonde recipient in a simple pink dress, exported once from the existing high-resolution master into a `28 × 44` canvas. At `x=290, y=324` her visible `15 × 40` silhouette stands beside the house at the upper edge of the nearest lane and remains smaller than the courier vehicle and destination architecture. She remains separate from the city texture so her depth and product-receiving anchor stay controllable.
