...
| `UI-018` | Delivery reward coupon background | `304 × 456`, static | integrated v6 |
...
`UI-018` v6 keeps the owner-supplied `1024 × 1536` JPEG and the v5 exterior-removal contract immutable. `scripts/build_reward_coupon_v6.py` writes a new binary-alpha master and one nearest-neighbor `304 × 456` runtime export. The upper section now carries the exact `BeautyBomb` wordmark in the existing UI-009 yellow/violet/pink treatment and centered Press Start 2P copy: `Спасибо за помощь нашему курьеру! Скопируйте подарочный промокод 20% на все товары.` The live code row moves below the supplied perforation: a `112 × 40` field at `(156,470)` and a `40 × 40` copy control at `(240,470)` remain optically centered inside the lower tear-off with a `44 × 44` interaction target. Clipboard fallback, confirmation, popup center `(180,320)`, origin, trigger, and cleanup are unchanged.
