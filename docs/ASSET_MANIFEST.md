...
Previous candidates remain as historical evidence. The active design master is `visual-references/veh-001-courier-clean-concept-v8-flip-top.png`, deterministically derived from immutable `v7` with only the rear cap region replaced: it preserves the turquoise side-view courier, roof mounts, and clean product silhouette while using a broad white flip-top cap with a hinge seam and recessed thumb-lift. The van and tube remain free of text and printed graphics.
...
The low-resolution `veh-001-courier-clean-static-v2.png` is retained only as diagnostic evidence of the rejected redraw pipeline. The active static master frame is `public/assets/game/vehicles/veh-001-courier-clean-static-v6.png` with adjacent metadata. It crops transparent padding from the versioned `v8` alpha master and performs exactly one nearest-neighbor resize into a `208 × 160` texture, then renders at `0.5` base scale so world size, origin, baselines, and collider behavior remain unchanged.

The integrated drive sheet is `public/assets/game/vehicles/veh-001-courier-clean-drive-v7.png` with adjacent metadata. Its four fixed-canvas frames are derived from the `v8` master, not from a preview or runtime file; frame zero is byte-identical to the clean static v6 texture. The intro idle sheet is `veh-001-courier-clean-intro-idle-v2.png`; it uses the same body/tube frames and frozen wheel hubs. Review guides: `visual-references/veh-001-courier-clean-static-v6-preview-4x.png` and `visual-references/veh-001-courier-clean-drive-v7-preview-4x.png`. The earlier files remain historical evidence and are not preloaded by the game.
...
| `ENV-010` | Lower control-panel cobblestone | `512 × 128` direct POT cycle / `360 × 122` clipped viewport with `2 px` road overlap and bottom overscan | integrated v1 |
...
`ENV-010` v1 is a compact deterministic `512 × 128` curb-gray cobblestone cycle, authored at runtime resolution by `scripts/build_control_panel_cobblestone_v1.py`. Its wrap-aware rows continue the same paver across the left/right edge; the script asserts zero edge mismatch, generates a three-copy seam review, uses no mirror, resize, smoothing, or palette quantization, and is rendered with `NEAREST`. Phaser clips the `360 × 122` TileSprite at `y=520`; it has a fixed `2 px` road overlap and a `2 px` bottom overscan past the logical canvas, preventing either a lower-panel underlay or a rounded FIT edge from appearing. It updates only through the existing environment lifecycle at a front-layer `1.28` multiplier, freezes under reduced motion, and never changes the UI controls' placement or touch areas.
...
| `UI-006` | Defeat popup callout | `332 × 272`, static | integrated (`v2`) |
...
| `UI-011` | Legacy top-left pixel exit placeholder | `32 × 32`, `2` idle/pressed frames | retained, superseded at runtime |
...
`UI-009` is the one-line `BEAUTY BOMB DELIVERY` gameplay title authored by the same deterministic HUD script. Its acid-yellow face, deep-violet outline, hot-pink lower-right extrusion, and small cyan-white highlights are drawn directly on a transparent `242 × 28` runtime grid. Phaser renders the PNG at native size with `NEAREST`, centered at `x=180, y=38`, six logical pixels below the utility-button centerline; no system font or runtime text effect is involved. Review evidence: `visual-references/ui-game-title-v1-review.png`. The legacy `UI-011` raster remains versioned but is not loaded: its current visual replacement is a direct anti-aliased HTML/CSS cross at logical center `x=28, y=32`, with two `2 px` red strokes, `1 px` black outlines, no background, and a transparent logical `44 × 44` pointer target. Percentage positioning and sizing keep the control aligned to the game canvas at every FIT scale without a raster export or resize path.
...
`UI-006` v2 is the defeat-only text sibling of `UI-015` v2. `scripts/build_defeat_callout_v2.py` reuses the pause panel's exact `332 × 272` geometry, palette, center `(180,318)`, Press Start 2P source, binary alpha, and zero-resize pipeline. It renders `ДТП! Давай еще раз! Внимательно следи за дорогой и не врезайся.` in five centered lines, all at the same `15 px` font size. The defeat overlay reuses the same dim layer, depth, and `createPauseMenuButton()` implementation; its single `заново` button remains at `(180,394)` and invokes the existing `resetRun()` flow. `UI-015` and the pause interaction remain unchanged.
...
`PRD-003 v3` supersedes the prior transfer asset in the active runtime. `scripts/build_delivery_product_v3.py` derives it in one nearest-neighbor resize from `visual-references/veh-001-courier-clean-concept-v8-flip-top.png`; after that single export, it clears only the measured `4 × 3 px` upper-left cap cast-shadow rectangle on the native hand-off canvas. The approved roof-tube master stays immutable, while the handed-off cream retains the same white flip-top, hinge seam, and thumb-lift without a cast-shadow treatment. The earlier v1 and v2 files remain historical and are not preloaded.
...
| `UI-018` | Delivery reward coupon background | `332 × 498`, static | integrated v7 |
| `PRD-003` | Vertical delivery-transfer cream | `32 × 64`, static | integrated v3 |

`DST-001`, `CHR-001`, and `CHR-003` retain separate immutable generated masters. The integrated recipient is `CHR-003` v1, built from the compact approved `112 × 176` master `visual-references/chr-003-lowpoly-recipient-master-v1.png`; `scripts/build_delivery_girl_lowpoly_v1.py` verifies its SHA-256 and performs one nearest-neighbor export to the final `28 × 44` canvas, avoiding repeated or fractional resizing. The previous `CHR-001` assets remain versioned in the repository and are not overwritten. The standalone house remains a design reference only. `ENV-009` v3 is derived by `scripts/build_delivery_destination_city_v3.py` from the immutable unified destination master `visual-references/env-009-delivery-destination-city-alpha-master-v2.png`; that master already contains the house as a complete city lot. The runtime export performs one nearest-neighbor resize, then copies the immediately adjacent approved curb sample into only the `42 × 6` doorstep interval with opaque alpha, so no sky-colored gap remains and no facade is regenerated. `UI-016` v5 is authored directly on its final pixel grid by `scripts/build_delivery_finale_ui_v5.py`; its existing tail geometry remains spatially aligned because `CHR-003` retains the approved `290,324` placement in this stage. The tail keeps the deep-violet outline and lavender interior but receives no pink shadow; the body retains its existing pink extrusion. `UI-017` remains the v1 three-frame sheet from `scripts/build_delivery_finale_ui_v1.py`. Both use the repository-local Press Start 2P source and no runtime font. `UI-018` v2 and its former live brand layer remain versioned for history but are no longer active. `PRD-003 v3` derives from the v8 master rather than a screenshot or runtime sprite. Finale timings, anchors, and depth ordering are adjacent in `src/game/content/deliveryFinale.json`. The destination frame is captured before `CHR-003` becomes visible, so the house doorway remains fixed behind her through the full `700 ms` `Sine.Out` reveal and no relative-motion jerk is introduced.

`UI-018` v7 retains the owner-supplied `1024 × 1536` JPEG and the v5 exterior-removal contract as immutable input. `scripts/build_reward_coupon_v7.py` writes a new binary-alpha master and one nearest-neighbor `332 × 498` runtime export: the coupon leaves `14 px` left/right and `71 px` top/bottom logical clearance at center `(180,320)`. Its `BeautyBomb` wordmark is redrawn at 17 px and the seven-line explanatory Press Start 2P copy at 12 px, grouped into complete phrases with one deliberate blank spacer between the thank-you and redemption instruction. The longest line is `204 px`, centered with `64 px` clearance from the coupon canvas; it remains inside the unornamented central surface. A large `20%` appears directly beneath that copy in the exact wordmark treatment at 40 px; its build gate rejects any width greater than the 169 px wordmark and its `y=270` placement preserves the gap to the lower tear-off. The live code row stays inside the lower tear-off as a `122 × 44` field at `(154,484)` and a `44 × 44` copy control at `(246,484)` with a `48 × 48` interaction target. Clipboard fallback, confirmation, trigger, and cleanup are unchanged.
