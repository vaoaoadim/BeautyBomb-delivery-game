...
`PRD-003 v2` supersedes the prior transfer asset in the active runtime. `scripts/build_delivery_product_v2.py` derives it in one nearest-neighbor resize from `visual-references/veh-001-courier-clean-concept-v8-flip-top.png`, so the handed-off cream retains the same white flip-top, hinge seam, and thumb-lift as the roof tube. The earlier v1 file remains historical and is not preloaded.
