...
| `UI-015` | Pause comic callout | `304 × 232`, static | integrated (`v1`) |
...
`UI-015` is a tail-free pause-window callout, authored directly at `304 × 232` by `scripts/build_pause_callout_v1.py`. It reuses the local SIL OFL Press Start 2P source and the integrated intro callout's lavender, dark-violet, white, and pink pixel treatment, but has no courier pointer. Its three rasterized lines read `Не тормози! Нужно успеть вовремя :)`; Phaser renders it with `NEAREST` at `x=180, y=312`. The existing continue and restart buttons remain unchanged and sit at `y=350` and `y=404` below the copy. No resize, antialiasing, runtime font load, or new pause-state is introduced.
