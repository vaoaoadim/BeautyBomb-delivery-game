...
| `UI-006` | Defeat popup callout | `332 × 272`, static | integrated (`v1`) |
...
`UI-006` v1 is the defeat-only text sibling of `UI-015` v2. `scripts/build_defeat_callout_v1.py` reuses the pause panel's exact `332 × 272` geometry, palette, center `(180,318)`, Press Start 2P source, binary alpha, and zero-resize pipeline, changing only the rasterized two-line copy to `ДТП! / Давай еще раз!`. The defeat overlay reuses the same dim layer, depth, and `createPauseMenuButton()` implementation; its single `заново` button occupies the pause restart-button center `(180,394)` and invokes the existing `resetRun()` flow. `UI-015` and the pause interaction remain unchanged.
...
| `UI-018` | Delivery reward coupon background | `304 × 456`, static | integrated v5 |
...
`UI-018` v5 fully replaces the v4 visual basis with the owner-supplied `1024 × 1536` JPEG stored as `visual-references/ui-018-reward-coupon-source-v5.jpeg`. `scripts/build_reward_coupon_v5.py` removes only its border-connected near-white exterior, preserving enclosed white and pale-yellow coupon details, and writes the binary-alpha master `visual-references/ui-018-reward-coupon-master-v5.png`. It adds only the exact `beautybomb` wordmark at logical top `y=100` in the existing UI-009 yellow/violet/pink title treatment, then performs one nearest-neighbor resize to `304 × 456`. The live code row, copy button, clipboard fallback, feedback, popup center `(180,320)`, origin, trigger, and cleanup remain unchanged. All v4 decorations and the separate lower tear-off text are absent; the area below the supplied perforation is empty.
