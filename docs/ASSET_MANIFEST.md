...
`UI-018` v7 retains the owner-supplied `1024 × 1536` JPEG and the v5 exterior-removal contract as immutable input. `scripts/build_reward_coupon_v7.py` writes a new binary-alpha master and one nearest-neighbor `332 × 498` runtime export: the coupon leaves `14 px` left/right and `71 px` top/bottom logical clearance at center `(180,320)`. Its `BeautyBomb` wordmark is redrawn at 17 px and the seven lines of explanatory Press Start 2P copy at 12 px, raising legibility without a runtime scale or second resize. The longest explanatory line is capped below `210 px`, keeping more than `60 px` of horizontal clearance from the coupon canvas. The live code row stays inside the lower tear-off as a `122 × 44` field at `(154,484)` and a `44 × 44` copy control at `(246,484)` with a `48 × 48` interaction target. Clipboard fallback, confirmation, trigger, and cleanup are unchanged.
