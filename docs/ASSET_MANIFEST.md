...
| `ENV-010` | Lower control-panel cobblestone | `512 × 128` direct POT cycle / `360 × 120` viewport with `2 px` road overlap | integrated v1 |
...
`ENV-010` v1 is a compact deterministic `512 × 128` curb-gray cobblestone cycle, authored at runtime resolution by `scripts/build_control_panel_cobblestone_v1.py`. Its wrap-aware rows continue the same paver across the left/right edge; the script asserts zero edge mismatch, generates a three-copy seam review, uses no mirror, resize, smoothing, or palette quantization, and is rendered with `NEAREST`. Phaser clips the viewport-sized TileSprite to `360 × 120` at `y=520`, giving it a fixed `2 px` road overlap that prevents any lower-panel underlay from appearing at the join. It updates only through the existing environment lifecycle at a front-layer `1.28` multiplier, freezes under reduced motion, and never changes the UI controls' placement or touch areas.
