...
def compose_master_frame(
    ...
    *,
    ...
    """Build one drive-motion frame, optionally keeping wheel hubs static."""
    if frame_index == 0:
        return source.copy()
    ...
    for wheel_box, hub_box in zip(WHEEL_BOXES, HUB_BOXES, strict=True):
        ...
        if rotate_hubs:
            ...
            hub = hub.transpose(rotation)
        frame.alpha_composite(hub, hub_box[:2])
    return frame
...
def save_intro_idle_metadata(frames: list[Image.Image]) -> None:
    metadata = {
        "id": "VEH-001",
        ...
        "production": {
            ...
            "approvedStaticRuntimeSha256": sha256(STATIC_OUTPUT),
            "buildScript": "scripts/build_courier_clean_asset.py",
            "exportMethod": "master-component drive motion with static source wheel hubs plus one production resize per frame",
            "assetMode": "high-detail-pixel-style-raster",
            "offlineResizeCountPerFrame": 1,
            "resizeFilter": "nearest-neighbor",
            "paletteQuantization": False,
            "phaserTextureFilter": "nearest",
            "runtimeSha256": sha256(IDLE_OUTPUT),
            "status": "produced-intro-idle",
        },
    }
    IDLE_METADATA.write_text(json.dumps(metadata, indent=2) + "\n", encoding="utf-8")
...
def main() -> None:
    ...
    frames = [
        render_foreground(
            compose_master_frame(master, frame_index, rotate_hubs=True)
        )
    ...
    idle_frames = [
        render_foreground(
            compose_master_frame(master, frame_index, rotate_hubs=False)
        )
        for frame_index in range(FRAME_COUNT)
    ]
    if idle_frames[0].tobytes() != static.tobytes():
        raise ValueError("Intro idle frame zero diverges from the clean static runtime")

    idle_sheet = Image.new(
        "RGBA",
        (CANVAS_SIZE[0] * FRAME_COUNT, CANVAS_SIZE[1]),
        (0, 0, 0, 0),
    )
    for frame_index, frame in enumerate(idle_frames):
        idle_sheet.alpha_composite(frame, (CANVAS_SIZE[0] * frame_index, 0))
    idle_sheet.save(IDLE_OUTPUT)
    save_intro_idle_metadata(idle_frames)
    idle_sheet.resize(
        (idle_sheet.width * PREVIEW_SCALE, idle_sheet.height * PREVIEW_SCALE),
        Image.Resampling.NEAREST,
    ).save(IDLE_PREVIEW)
