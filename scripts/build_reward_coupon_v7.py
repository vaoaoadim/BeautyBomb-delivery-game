"""Build UI-018 v7 with a larger, readable reward-coupon composition.

The owner-provided ticket source remains the only visual source. This version
renders the wordmark and informational copy at a larger native grid, then
performs one nearest-neighbor export for Phaser.
"""
...
MASTER = ROOT / "visual-references/ui-018-reward-coupon-master-v7.png"
RUNTIME = ROOT / "public/assets/game/ui/ui-018-reward-coupon-v7.png"
...
def connected_exterior_mask(source: Image.Image) -> Image.Image:
    """Return the border-connected near-white background mask only."""
...
def add_information_copy(master: Image.Image) -> dict[str, int]:
    scale = master.width / RUNTIME_SIZE[0]
    font = ImageFont.truetype(FONT_PATH, round(INFO_FONT_SIZE_LOGICAL_PX * scale))
    ...
    for index, line in enumerate(INFO_LINES):
        ...
        width = bbox[2] - bbox[0]
        x = round((master.width - width) / 2 - bbox[0])
        y = round((INFO_TOP_LOGICAL_PX + index * INFO_LINE_HEIGHT_LOGICAL_PX) * scale - bbox[1])
        draw.text((x, y), line, font=font, fill=255)
        logical_widths.append(round(width / scale))

    mask = threshold_mask(mask)
    master.alpha_composite(colorize(mask, PALETTE["violet"]))
    return {
        "x": round((RUNTIME_SIZE[0] - max(logical_widths)) / 2),
        "y": INFO_TOP_LOGICAL_PX,
        "width": max(logical_widths),
        "height": (len(INFO_LINES) - 1) * INFO_LINE_HEIGHT_LOGICAL_PX + INFO_FONT_SIZE_LOGICAL_PX,
    }
...
def assert_binary_alpha(image: Image.Image) -> None:
    ...
    if sum(alpha[1:255]):
        raise RuntimeError("UI-018 v7 contains non-binary alpha values")


def main() -> None:
    if file_hash(SOURCE) != SOURCE_SHA256:
        raise RuntimeError("UI-018 v7 source differs from the owner-provided artwork")
    ...
    with Image.open(SOURCE) as supplied:
        if supplied.size != SOURCE_SIZE:
            raise RuntimeError(f"Unexpected UI-018 v7 source size: {supplied.size}")
    ...
    metadata = {
        ...
        "master": {
            "path": "visual-references/ui-018-reward-coupon-master-v7.png",
        ...
        "runtime": {
            "path": "public/assets/game/ui/ui-018-reward-coupon-v7.png",
            "sha256": file_hash(RUNTIME),
            "pixelSha256": pixel_hash(runtime),
            "dimensions": {"width": RUNTIME_SIZE[0], "height": RUNTIME_SIZE[1]},
        ...
        "liveOverlay": {
            ...
            "codeAndCopyControl": "compact live Phaser and DOM accessibility UI in the lower tear-off",
            "tearOffArea": "contains only the code row, copy control, and transient copy confirmation",
            "codeField": {"x": 154, "y": 484, "width": 122, "height": 44},
            "copyButton": {"x": 246, "y": 484, "width": 44, "height": 44, "hitArea": 48},
        },
    }
