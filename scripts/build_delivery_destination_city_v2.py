"""Build ENV-009 v2 from the generated unified destination-city master."""
...
OUTPUT = ROOT / "public/assets/game/environment/env-009-delivery-destination-city-v2.png"
...
def main() -> None:
    if file_hash(MASTER) != APPROVED_MASTER_SHA256:
        raise RuntimeError("Approved ENV-009 v2 alpha master changed")
    if file_hash(CHROMA_MASTER) != APPROVED_CHROMA_SHA256:
        raise RuntimeError("Approved ENV-009 v2 chroma master changed")

    master = Image.open(MASTER).convert("RGBA")
    if master.size != (2172, 724):
        raise RuntimeError(f"Unexpected ENV-009 v2 master canvas: {master.size}")
    ...
    write_json(
        OUTPUT.with_suffix(".json"),
        {
            ...
            "composition": "One generated city panorama with DST-001 architecture built into a complete foreground lot",
            "runtime": {
            ...
            "production": {
                "buildScript": "scripts/build_delivery_destination_city_v2.py",
