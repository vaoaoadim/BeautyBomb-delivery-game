"""Build UI-016 v4 from the proven UI-013 comic-tail construction."""
...
CALLOUT_FILE = OUTPUT / "ui-016-delivery-callout-v4.png"
CALLOUT_SIZE = (332, 104)
RUNTIME_POSITION = {"x": 14, "y": 350, "originX": 0, "originY": 0}
...
def assert_binary_alpha(image: Image.Image) -> None:
    ...
    if sum(histogram[1:255]) != 0:
        raise RuntimeError("UI-016 v4 contains antialiased alpha values")
...
def build_callout(font: ImageFont.FreeTypeFont) -> Image.Image:
    ...
    # Exact UI-013 tail silhouette reflected upward and translated so its
    # point lands at local x=206. Drawing the tail after the body hides the
    # body's top edge at the join, exactly as the approved intro asset does.
    tail_outer = [
        (180, 34),
        (225, 34),
        (221, 21),
...
def main() -> None:
    ...
    write_json(
        CALLOUT_FILE.with_suffix(".json"),
        {
            ...
            "visibleBottomAtRuntime": RUNTIME_POSITION["y"] + 104,
            "production": {
                "buildScript": "scripts/build_delivery_finale_ui_v4.py",
