"""Build the v3 intro callout with the owner-requested yellow mark removed.

UI-013 v1 remains immutable. This small deterministic derivation preserves the
approved 332 x 207 callout pixels and restores only the yellow decorative mark
to the lavender thought-bubble surface in the lower-right interior.
"""
...
TARGET = OUTPUT / "ui-013-intro-callout-v3.png"
...
def main() -> None:
    ...
    # bounds are x=300..313, y=158..160; replace exact yellow pixels with
    # the opaque lavender bubble surface so nothing behind the callout shows.
    removed_pixels: list[tuple[int, int]] = []
    for y in range(158, 161):
        for x in range(300, 314):
            if image.getpixel((x, y)) == YELLOW:
                image.putpixel((x, y), LAVENDER)
    ...
    metadata = {
        ...
        "change": "Replaced the yellow decorative mark with the opaque lavender thought-bubble surface.",
        "replacedLocalRect": {"x": 300, "y": 158, "width": 14, "height": 3},
        "replacedPixelCount": len(removed_pixels),
