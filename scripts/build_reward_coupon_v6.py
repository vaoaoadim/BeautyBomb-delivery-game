"""Build UI-018 v6 from the owner-provided coupon artwork.
...
``BeautyBomb`` wordmark and the approved thank-you copy are then added before
the master is resized once with nearest-neighbor for Phaser.
"""
...
MASTER = ROOT / "visual-references/ui-018-reward-coupon-master-v6.png"
RUNTIME = ROOT / "public/assets/game/ui/ui-018-reward-coupon-v6.png"
...
TITLE_TOP_LOGICAL_PX = 86
INFO_LINES = (
    "Спасибо за помощь",
    "нашему курьеру!",
    "Скопируйте подарочный",
    "промокод 20%",
    "на все товары.",
)
INFO_TOP_LOGICAL_PX = 132
INFO_FONT_SIZE_LOGICAL_PX = 9
...
def assert_binary_alpha(image: Image.Image) -> None:
    ...
    if sum(alpha[1:255]):
        raise RuntimeError("UI-018 v6 contains non-binary alpha values")


def main() -> None:
    if file_hash(SOURCE) != SOURCE_SHA256:
        raise RuntimeError("UI-018 v6 source differs from the owner-provided artwork")
    ...
    with Image.open(SOURCE) as supplied:
        if supplied.size != SOURCE_SIZE:
            raise RuntimeError(f"Unexpected UI-018 v6 source size: {supplied.size}")
