"""Build CHR-003 v1 from the approved compact low-poly master."""
...
MASTER = ROOT / "visual-references/chr-003-lowpoly-recipient-master-v1.png"
OUTPUT = ROOT / "public/assets/game/characters/chr-003-lowpoly-recipient-v1.png"
...
def main() -> None:
    if file_hash(MASTER) != APPROVED_MASTER_SHA256:
        raise RuntimeError("Immutable CHR-003 master changed")

    source = Image.open(MASTER).convert("RGBA")
    alpha = source.getchannel("A").point(lambda value: 255 if value >= 16 else 0)
    bbox = alpha.getbbox()
    if bbox is None:
        raise RuntimeError("CHR-003 master has no visible pixels")
