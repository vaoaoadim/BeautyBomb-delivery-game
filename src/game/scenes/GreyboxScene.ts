...
const HUD_ASSETS = Object.freeze({
  ...
  pause: {
    textureKey: "hud-pause-button-v1",
    path: "/assets/game/ui/ui-010-pause-button-v1.png",
    frameWidth: 32,
    frameHeight: 32,
    x: 332,
    y: 32,
  ...
  exit: {
    textureKey: "hud-exit-button-v1",
    path: "/assets/game/ui/ui-011-exit-button-v1.png",
    frameWidth: 32,
    frameHeight: 32,
    x: 28,
    y: 32,
  ...
  sound: {
    textureKey: "hud-sound-tab-v1",
    path: "/assets/game/ui/ui-012-sound-tab-v1.png",
    frameWidth: 28,
    frameHeight: 28,
    x: 346,
    y: 540,
...
export class GreyboxScene extends Phaser.Scene {
  ...
  public preload(): void {
    ...
    this.load.spritesheet(HUD_ASSETS.exit.textureKey, HUD_ASSETS.exit.path, {
      frameWidth: HUD_ASSETS.exit.frameWidth,
      frameHeight: HUD_ASSETS.exit.frameHeight,
    });
  ...
  public create(): void {
    ...
    this.createPlaceholderControls();
    ...
    this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
      ...
      this.stopPlayerIdleTween(false);
  ...
  private createPauseControl(): void {
    ...
    this.pauseButton = this.add
      ...
      .setDepth(RENDER_DEPTH.controls)
      .setScrollFactor(0)
      .setVisible(false)
      .setInteractive({ useHandCursor: true });
  ...
  private createPlaceholderControls(): void {
    this.exitButton = this.createPlaceholderControl(
      HUD_ASSETS.exit.x,
      HUD_ASSETS.exit.y,
      HUD_ASSETS.exit.textureKey,
    );
    this.soundButton = this.createPlaceholderControl(
      HUD_ASSETS.sound.x,
      HUD_ASSETS.sound.y,
      HUD_ASSETS.sound.textureKey,
    );
  ...
  private createPlaceholderControl(
    x: number,
    y: number,
    textureKey: string,
  ): Phaser.GameObjects.Sprite {
  ...
  private setUtilityControlsVisible(visible: boolean): void {
    this.pauseButton.setFrame(0).setVisible(visible);
    this.exitButton.setFrame(0).setVisible(visible);
    this.soundButton.setFrame(0).setVisible(visible);
  }
  ...
  private startPlayerIdleTween(): void {
    if (
      this.prefersReducedMotion ||
      this.runState.phase !== "ready" ||
      this.playerIdleTween
    ) {
      return;
    }

    const baseline = LANE_BASELINES[this.runState.lane];
    this.player.setY(baseline);
  ...
  private stopPlayerIdleTween(restoreBaseline: boolean): void {
    const idleTween = this.playerIdleTween;
  ...
  private completeIntroTransition(): void {
    ...
    this.player.play(PLAYER_DRIVE_ANIMATION);
